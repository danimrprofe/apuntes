...
Lego Mindstorms NXT és una plataforma de robòtica educativa que permet als usuaris construir robots programables amb peces de LEGO i controlar-los a través d'un microcontrolador programable.

![](img/2023-04-09-09-12-50.png)
...
## Màquines, autòmats i robots

Una ``màquina`` és un dispositiu que utilitza energia per a realitzar una tasca. Les màquines poden ser controlades per una persona, i són dissenyades per a realitzar una tasca específica.

![](img/2023-04-09-09-14-49.png)

Els ``autòmats``, també coneguts com a màquines automàtiques, són màquines que són capaces de realitzar una tasca sense la intervenció d'un usuari. Un exemple d'autòmat és una màquina expenedora, que lliura productes a canvi de monedes o bitllets.

![](img/2023-04-09-09-15-25.png)

El ``robot`` és un autòmat programable que és capaç de realitzar tasques complexas i variades. Els robots són dissenyats per a imitar les accions humanes, i poden ser utilitzats en molts sectors diferents, com la fabricació, la medicina, l'agricultura, i molts altres. Els robots poden ser programats per a realitzar tasques específiques o adaptar-se a noves situacions i prendre decisions autònomes en funció de les circumstàncies.

En resum, mentre que una màquina és controlada per una persona, un autòmat és capaç de funcionar sense intervenció humana, i un robot és capaç de prendre decisions i adaptar-se a noves situacions en funció dels programes que té preinstal·lats.
...
El brick de LEGO Mindstorms NXT compta amb un coprocessador Atmel AVR de 8 bits i dos tipus de memòria:

- ``Memòria flaix``: és on s'emmagatzema el microprogramari del sistema operatiu i les aplicacions programades per l'usuari. En el cas de NXT, té una capacitat de **256 KB**.
- ``Memòria RAM``: és on s'emmagatzemen les dades temporals i les variables utilitzades en el temps d'execució. En el cas de NXT, té una capacitat de **64 KB**.

## Bateries

Les bateries recarregables utilitzades al LEGO Mindstorms NXT proporcionen una capacitat de **2.200 mAh**.

La càrrega de les bateries es fa a una **tensió de 9V i un corrent d'uns 500 mA**. És important tenir en compte que el procés de càrrega de les bateries ha de ser supervisat per evitar sobrecarregar-les, cosa que podria danyar-les i reduir la seva vida útil.
