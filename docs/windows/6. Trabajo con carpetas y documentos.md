...
Cualquier cosa que guardemos en el ordenador se guarda en forma de archivo. Podemos consultar cuánto ocupan. En este caso, este archivo contiene 3,45 KB o Kilobytes de información.
...
El tiempo se calcula como espacio / tiempo:
...
Haz doble clic en el ícono de la Papelera de Reciclaje en Windows.

Ubica y selecciona los archivos y carpetas que necesitas restaurar.

Haz clic o clic derecho en el botón "Restaurar".
...
``Ctrl`` + X permite cortar un archivo o carpeta, que se queda en el portapapeles hasta que decidamos pegarlo en otro sitio. De esta forma, movemos la carpeta o el archivo de lugar.
...
Copiar con ``Ctrl`` apretado, arrastrar el archivo y soltar el ratón

``Ctrl`` + C y ``Ctrl`` + V donde queramos pegarlo
...
Podemos seleccionar archivos no consecutivos. Para ello, mantener ``Ctrl`` apretado y hacer clic en los elementos que queremos seleccionar.

![imagen](media/image16.gif)

Crea un documento de texto con tu nombre, y haz 10 copias de él. Adjunta una captura.
